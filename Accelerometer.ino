...
void loop()
{
  ...
  dX = ((int16_t) x / 64.0) * G;          // calculate the 'g' values.
  dY = ((int16_t) y / 64.0) * G;
  dZ = ((int16_t) z / 64.0) * G;
  ...
  if (dX < -5.0) Serial.println("Tilted LEFT");
  if (dX >  5.0) Serial.println("Tilted RIGHT");
  if (dY < -5.0) Serial.println("Tilted TOWARDS");
  if (dY >  5.0) Serial.println("Tilted AWAY");
  if (dZ < -8.0) Serial.println("UPSIDE DOWN");
